...
async function fetchAndApplyProxy() {
  ...
  try {
    ...
    let apiUrl = `${API_BASE_URL}/proxies/extension/get-proxy-by-key?keyxoay=${encodeURIComponent(keyxoay)}`;
    ...
    if (data.data.status === 101) {
      ...
      if (timeLeftMatch && timeLeftMatch[1]) {
        ...
        const waitTime = new Date(proxyInfo.nextRotationTime).toLocaleTimeString();
        ...
        console.log(`Chưa đến thời gian xoay proxy, còn ${timeLeftSeconds} giây`);
    ...
    if (data.data.status === 100) {
      ...
      if (data.data.proxyhttp && data.data.proxysocks5) {
        ...
        const proxyString = proxyInfo.proxyType === 'http' ? httpString : socks5String;
    ...
    // Các trạng thái khác
    throw new Error(`API trả về trạng thái không xử lý được: ${data.data.status}`);
...
function notifyPopupProxyUpdated() {
  chrome.runtime.sendMessage({
    ...
    if (chrome.runtime.lastError) {
      console.log('Popup chưa mở, không thể gửi thông báo:', chrome.runtime.lastError.message);
    }
  });
}
...
function applyProxyToChrome(proxyData) {
  ...
  try {
    ...
    let displayIsp = proxyData.nhaMang || (proxyInfo.isp !== 'random' ? proxyInfo.isp.toUpperCase() : '');
    let displayProvince = proxyData.viTri || (proxyInfo.province !== 'random' ? proxyInfo.province : '');
    let ispProvince = '';
    ...
    let expirationInfo = proxyData.expiration ? ` | Hết hạn: ${proxyData.expiration}` : '';
    proxyInfo.currentProxyInfo = `${proxyData.host}:${proxyData.port} [${proxyData.type.toUpperCase()}] (${ispProvince})${expirationInfo}`;
  ...
  const setupNextRotation = () => {
    if (proxyInfo.nextRotationTime) {
      ...
      if (timeLeft > 0) {
        ...
        console.log(`Đặt timer xoay proxy sau ${Math.ceil(timeLeft/1000)} giây`);
        ...
        fetchAndApplyProxy()
          .then(proxyData => {
            if (proxyData) {
              ...
              console.log('Dừng timer hiện tại và thiết lập timer mới cho thời gian chính xác');
              stopProxyRotation();
              setupNextRotation();
            }
          })
...
chrome.storage.sync.get(['proxyKey', 'rotationTime', 'autoAssign', 'isActive', 'isp', 'province', 'proxyType', 'currentProxyInfo', 'proxyData'], function(result) {
